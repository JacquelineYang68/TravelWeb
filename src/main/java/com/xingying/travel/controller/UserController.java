...
public class UserController {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/forget/{vercode}",method = RequestMethod.POST)
	public Result forget(@PathVariable("vercode")String vercode,@RequestBody User user){
		//得到redis缓存中的验证码
		String checkcodeRedis =(String) redisTemplate.opsForValue().get("checkcode_");
		...
		if (!checkcodeRedis.equals(vercode)){
			return new Result(false,StatusCode.ERROR,"验证码错误");
		}else {
